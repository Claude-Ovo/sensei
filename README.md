...
- A background **Observer** agent (Gemini 3.7 Flash via ADK) reads the messy stream — ANSI, stack traces, progress bars — and decides, on evidence, whether you're flowing, exploring, or stuck. It stays silent by default. When it speaks, it's one line, in your language, at the level you asked for.
...
```bash
git clone <this repo> sensei && cd sensei
npm install
npm run build
