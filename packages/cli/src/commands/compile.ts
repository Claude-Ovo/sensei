...
export async function compile(sessionId: string | undefined, opts: { offline?: boolean; print?: boolean }) {
  ...
  for (const c of chunks) {
    ...
    if (c.kind === 'agent') {
      ...
      else if (kind === 'question') pendingQ = c.text;
      else if (kind === 'answer' && pendingQ) {
        qa.push({ q: pendingQ, a: c.text });
        pendingQ = null;
      }
      continue;
    }
    if (c.kind === 'user') {
      ...
      else if (kind === 'ask') pendingQ = c.text;
      else if (kind === 'reply' && pendingQ) {
        qa.push({ q: pendingQ, a: `(learner) ${c.text}` });
        pendingQ = null;
      }
      transcript.push(c);
      continue;
